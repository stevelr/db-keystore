...
fn cmd_list(
    ...
    let entries = store.search(&spec).map_err(map_keyring_err)?;
...
fn cmd_delete(
    ...
    match (args.uuid, args.service, args.user) {
        ...
        _ => Err(
            "delete requires either --uuid UUID or -s SERVICE -u USER (and no extra args)"
                .to_string(),
        ),
    }
}
...
fn cmd_rekey(
    ...
    if encrypted {
        // If omitted, encryption is removed.
    } else if new_cipher_opts.is_none() {
        return Err(
            "database is not encrypted; --new-cipher and --new-hexkey are required".to_string(),
        );
    }
    ...
    let new_store = DbKeyStore::new(&config).map_err(map_keyring_err)?;

    let entries = store.search(&HashMap::new()).map_err(map_keyring_err)?;
...
fn validate_cipher_and_key(
    ...
    match (cipher, hexkey) {
        ...
        (Some(cipher), Some(hexkey)) => {
            ...
            let expected_len =
                expected_hex_len(&cipher).ok_or_else(|| format!("invalid cipher '{cipher}'"))?;
            if hexkey.len() != expected_len {
                return Err(format!(
                    "hexkey length must be {expected_len} for cipher '{cipher}'"
                ));
            }
            if !hexkey.chars().all(|c| c.is_ascii_hexdigit()) {
                return Err("hexkey must be valid hex".to_string());
            }
            Ok(Some((cipher, Zeroizing::new(hexkey))))
        }
        _ => Err("cipher and hexkey must be provided together".to_string()),
    }
}
...
fn resolve_path(path: Option<PathBuf>) -> Result<PathBuf, String> {
    match path {
        Some(path) => Ok(path),
        None => db_keystore::default_path().map_err(|e| e.to_string()),
    }
}

fn ensure_db_exists(path: &Path) -> Result<(), String> {
    if !path.is_file() {
...
fn open_store(
    ...
    let store = DbKeyStore::new_with_modifiers(&modifiers_ref).map_err(map_keyring_err)?;
...
fn entries_to_rows(entries: Vec<Entry>) -> Result<Vec<ListRow>, String> {
    let mut rows = Vec::with_capacity(entries.len());
    for entry in entries {
        let (service, user) = entry
            .get_specifiers()
            .ok_or_else(|| "missing specifiers".to_string())?;
        let attrs = entry.get_attributes().map_err(map_keyring_err)?;
        let uuid = attrs
            .get("uuid")
            .cloned()
...
fn output_rows(rows: &[ListRow], json: bool) -> Result<(), String> {
    if json {
...
fn delete_by_uuid(store: &DbKeyStore, uuid: &str) -> Result<i32, String> {
    ...
    match entry.delete_credential() {
        ...
        Err(KeyringError::Ambiguous(_)) => Err("Ambiguous".to_string()),
        Err(err) => Err(map_keyring_err(err)),
    }
}

fn delete_by_service_user(store: &DbKeyStore, service: &str, user: &str) -> Result<i32, String> {
    let entry = store.build(service, user, None).map_err(map_keyring_err)?;
    match entry.delete_credential() {
        Ok(()) => {
            println!("Deleted");
            Ok(0)
...
fn normalize_uuid(value: &str) -> Result<String, String> {
    let uuid = uuid::Uuid::parse_str(value).map_err(|_| "invalid uuid format".to_string())?;
    Ok(uuid.to_string())
}

fn copy_entry(entry: &Entry, new_store: &DbKeyStore) -> Result<(), String> {
    let (service, user) = entry
        .get_specifiers()
        .ok_or_else(|| "missing specifiers".to_string())?;
    let attrs = entry.get_attributes().map_err(map_keyring_err)?;
    ...
    let new_entry = new_store
        .build(service.as_str(), user.as_str(), Some(&mods))
        .map_err(map_keyring_err)?;

    let secret = Zeroizing::new(entry.get_secret().map_err(map_keyring_err)?);
    new_entry
...
fn open_db(
    ...
    block_on(builder.build()).map_err(|e| e.to_string())
}
...
fn remove_unique_index_or_rebuild(conn: &Connection) -> Result<(), String> {
...
fn find_unique_service_user_index(conn: &Connection) -> Result<Option<String>, String> {
    ...
    while let Some(row) = block_on(rows.next()).map_err(|e| e.to_string())? {
        let name = value_to_string(row.get_value(0).map_err(|e| e.to_string())?, "index name")?;
        let sql = value_to_string(row.get_value(1).map_err(|e| e.to_string())?, "sql")?;
...
fn table_has_unique_service_user(conn: &Connection) -> Result<bool, String> {
    let mut rows = block_on(conn.query(
        "SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'credentials' AND sql IS NOT NULL",
        (),
    ))
    .map_err(|e| e.to_string())?;
    if let Some(row) = block_on(rows.next()).map_err(|e| e.to_string())? {
        let sql = value_to_string(row.get_value(0).map_err(|e| e.to_string())?, "sql")?;
...
fn rebuild_credentials_without_unique(conn: &Connection) -> Result<(), String> {
    block_on(conn.execute("BEGIN IMMEDIATE", ())).map_err(|e| e.to_string())?;
    let result = (|| {
        ...
        .map_err(|e| e.to_string())?;
        block_on(conn.execute("DROP TABLE credentials", ())).map_err(|e| e.to_string())?;
    ...
    match result {
        Ok(()) => {
            block_on(conn.execute("COMMIT", ())).map_err(|e| e.to_string())?;
            Ok(())
        }
        Err(err) => {
...
fn value_to_string(value: Value, field: &str) -> Result<String, String> {
    match value {
        Value::Text(text) => Ok(text),
        Value::Blob(blob) => {
            String::from_utf8(blob).map_err(|e| format!("invalid utf8 for {field}: {e}"))
        }
        other => Err(format!("unexpected value for {field}: {other:?}")),
    }
}

fn map_keyring_err(err: KeyringError) -> String {
    err.to_string()
}
